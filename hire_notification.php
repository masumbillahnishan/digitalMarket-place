...
$sql = "SELECT * FROM hire WHERE email = '$email' ";
    ...
    <main class="flex mx-auto">
        ...
        <section class=" backdrop-opacity-60 backdrop-grayscale bg-white/60 p-8 rounded-lg shadow-md w-96 h-96 border border-2 border-cyan-300 mx-auto my-12">
            <div class="text-center">
                <h1 class="text-2xl font-semibold text-cyan-700 mb-4"> Information</h1>
            </div>

            <div class="p-4">
                <div class="mb-4 flex">
                    <label class="block text-gray-700 font-semibold" for="seller-name"> Name:</label>
                    <p class="text-gray-800 border border-2 border-cyan-500 px-4" id="seller-name"><?php echo $name; ?></p>
                </div>

                <div class="mb-4 flex">
                    <label class="block text-gray-700 font-semibold" for="seller-address">Address:</label>
                    <p class="text-gray-800  border border-2 border-cyan-500 px-4" id="seller-address"><?php echo $address; ?></p>
                </div>

                <div class="mb-4 flex">
                    <label class="block text-gray-700 font-semibold" for="seller-email">Email:</label>
                    <p class="text-gray-800  border border-2 border-cyan-500 px-4" id="seller-email"><?php echo $userEmail; ?></p>
                </div>
                <div class="flex">
                    <label class="block text-gray-700 font-semibold" for="seller-id">ID:</label>
...
$sqlG = "SELECT * FROM `hiredetails` where id='$id'";
$ck = mysqli_query($conn, $sqlG);

if ($ck) {
    echo '<h1 class="text-4xl font-bold text-center mb-6">Hire Notification</h1>';
    echo '<div class="grid grid-cols-4 gap-4 font-bold text-red-600">';
